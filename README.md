...
It opens on the body alone; switch the cut on and its controls — the axis, the position, the sweep, the snap — appear with it, as the body's Density and Focus go with the body. Both together is the picture the mode is for: a bright section moving through a ghost. The body stops at the cut on the eye's side, so the section reads as the face of a solid you have opened; neither is outlined, and the tetrahedron's own edges are drawn only when the cut is shown by itself, so it has the shape it was cut from to be read against. **Sweep the cut** moves it back and forth through the whole shape on its own, a pass every eight seconds, with the slider following.

The field is drawn in WebGL2 (a filtered 3D texture on the card); the colormap and the theme apply to it as to everything else. **Type/Setting › None** takes the points away to leave the field alone — and the Complexity channels, which size and colour the points, sit under Type/Setting in this mode and go with them.
...
* **Halftone** (under the swatches, in all three modes) — the picture in one ink. A screen of **Dots**, of **Lines** or of **Cross**-hatching says the value by how much of each cell it inks, on a ground that is pure black (night) or pure white (day) with the ink the other; everything drawn on it — the lattice, the labels, the cursor, the contours, the tetrahedron's points — is drawn in the ink. It stands in for whichever colormap is chosen (the chips dim while it is on), and **Size** is the pitch of the screen in pixels. Each cell reads the value at its centre: a dot's *area* is the value, a hatch's *width* is, and the two hatches of a cross are weighted so their union covers exactly it. The screen is laid at 45°, as printed halftones are. On the lifted surfaces — the triangle's relief, the tetrahedron's cut and body — it can be laid two ways, and **Screen | Shape** chooses: on the *screen* the lattice is fixed to the glass, so the dots stay the size and place the slider says however the shape is turned and the surface shows through them; on the *shape* the lattice is laid on the surface itself, turning, tilting and foreshortening with it, so the flat pane's own screen is seen in perspective. In the tetrahedron's body the shape lay is a three-dimensional lattice: each cell holds a globe (or a rod, for Lines and Cross) sized by the concordance at its point, scattered a little within its cell, and the ray is walked through them cell by cell — so a well of concordance appears as a cluster, packed solid along the chains of simplest chords and thinning to a sprinkle of small stars around them, with Focus and Density setting how sharply it resolves and how much ink it carries. Laid on the shape, Size is the pitch at the view the pane opens on, and the marks then grow with the zoom. The exports come out as vectors — a circle per dot, a path per hatch, no raster in the file — which is the point: a textbook's figure rather than a screen's.
